...
export default function NodeCanvas({ registerApi }) {
  ...
  // ---- tiny change event system (so GraphApp can mark tabs dirty) ----
  const changeSubs = useRef(new Set());
  const emitChange = () => { for (const f of changeSubs.current) { try { f(); } catch {} } };
  useEffect(() => { emitChange(); }, [nodes, links, view]);
  ...
  // --------- PUBLIC API (used by GraphApp per-tab view persistence) ---------
  useEffect(() => {
    const api = {
      ...
      exportJSON() {
        return {
          ...
          nodes: nodes.map(n => ({
            id:n.id, title:n.title, type:n.meta?.type||null, x:n.x, y:n.y, w:n.w,
            fields:n.fields, inputs:n.inputs, outputs:n.outputs
          })),
          links
        };
      },
      importJSON(doc) {
        ...
        setNodes(doc.nodes.map(n => ({
          id:n.id, title:n.title||"Node", meta:{type:n.type||"Imported"},
          x:n.x, y:n.y, w:n.w||NODE_W, fields:n.fields||{},
          inputs:n.inputs||[{id:"i0",name:"in"}], outputs:n.outputs||[{id:"o0",name:"out"}],
          selected:false
        })));
      ...
      // ---- Zoom/Pan API (GraphApp reads/writes per tab) ----
      getZoom() { return view.k; },
      getPan() { return { x: view.x, y: view.y }; },
      getView() { return { zoom: view.k, pan: { x: view.x, y: view.y } }; },
      setZoom(k) { zoomTo(k, centerPoint()); },
      setPan(p) { setView(v => ({ ...v, x: p?.x ?? v.x, y: p?.y ?? v.y })); },
      panTo(p) { setView(v => ({ ...v, x: p?.x ?? v.x, y: p?.y ?? v.y })); },
      setView(v) {
        if (!v) return;
        if (typeof v.zoom === "number") api.setZoom(v.zoom);
        if (v.pan) api.setPan(v.pan);
      },

      // ---- “Show Entire Workspace” + Zoom controls ----
      showAll: fitToContent,
      fit: fitToContent,
      fitToScreen: fitToContent,
      fitView: fitToContent,
      viewAll: fitToContent,
      zoomFit: fitToContent,

      zoomIn() { zoomAt(1.1, centerPoint()); },
      zoomOut() { zoomAt(1/1.1, centerPoint()); },

      // change subscription (GraphApp uses this to mark dirty)
      onChange: {
        subscribe(fn) {
          if (typeof fn !== "function") return () => {};
          changeSubs.current.add(fn);
          return () => changeSubs.current.delete(fn);
        }
      }
    };
    registerApi?.(api);
  ...
  const getPortCenterWorld = (type, nid, pid) => {
    const scr = getPortCenterScreen(type, nid, pid); if (!scr) return null;
    const rect = wrapRef.current.getBoundingClientRect();
    return { x: (scr.x - rect.left - view.x) / view.k, y: (scr.y - rect.top - view.y) / view.k };
  };

  // ===== Wire path computation with post-layout (double rAF) to avoid "floating" =====
  const [wirePaths, setWirePaths] = useState([]);
  const rafRefs = useRef({ a: 0, b: 0 });
  function computeWirePaths() {
    const paths = [];
  ...
  function scheduleWireRecalc() {
    const r = rafRefs.current;
    if (r.a) cancelAnimationFrame(r.a);
    if (r.b) cancelAnimationFrame(r.b);
    // Wait two frames to allow React style/transform to commit before measuring
    r.a = requestAnimationFrame(() => {
      r.b = requestAnimationFrame(() => setWirePaths(computeWirePaths()));
    });
  }
  useEffect(() => { scheduleWireRecalc(); }, [nodes, links, wip, view]);
  useEffect(() => {
    const onResize = () => scheduleWireRecalc();
    window.addEventListener("resize", onResize);
    return () => {
      window.removeEventListener("resize", onResize);
      const { a, b } = rafRefs.current;
  ...
  function zoomTo(kTarget, pivotScreen) {
    setView(v => {
      const k = Math.min(kMax, Math.max(kMin, kTarget));
  ...
  function zoomAt(factor, pivotScreen) {
    setView(v => {
  ...
  function onCanvasMouseDown(e) {
    if (e.target.closest?.(".yc-node") || e.target.closest?.(".yc-port")) return;
  ...
  function startDragFromNode(e, nid) {
    ...
    const offsets = moving.map(id => { const node = nodeById(id); return { id, dx: start.x - node.x, dy: start.y - node.y }; });
    setDrag({ ids: moving, offsets }); e.stopPropagation();
  }
  useEffect(() => {
    const onMove = (e) => {
      if (!drag) return;
  ...
  function startWire(fromNid, outPid, ev) {
    const el = document.querySelector(`[data-port-key="out:${fromNid}:${outPid}"] .dot`); if (!el) return;
    const r = el.getBoundingClientRect();
    setWip({ from: { nid: fromNid, pid: outPid }, x: r.left + r.width / 2, y: r.top + r.height / 2 });
    ev.stopPropagation();
  }
  ...
  function nearestInputPortWorld(xw, yw) {
    ...
    let best = null, bestD2 = 256;
    for (const el of ports) {
      const k = el.getAttribute('data-port-key'); const [, nid, pid] = k.split(':');
      const c = getPortCenterWorld('in', nid, pid); if (!c) continue;
  ...
  function acceptWire(toNid, inPid, ev) {
    ...
    const id = "l" + uid(); const link = { id, from: wip.from, to: { nid: toNid, pid: inPid } };
    setLinks(L => [...L.filter(x => !(x.to.nid === toNid && x.to.pid === inPid)), link]);
    setWip(null); ev.stopPropagation();
  }
  function endWire() { if (wip) setWip(null); }

  // ---- ORIGINAL “Show Entire Workspace” (fit to content) ----
  function fitToContent(){
    ...
    setView({x,y,k});
    // Wire recompute will happen via [view] effect with double rAF
  }
  ...
  return (
    ...
    >
      ...
      {/* Toolbar: Pan | Zoom Out | % | Zoom In | Fit */}
      <div className="yc-toolbar" style={{ position:"absolute", right:12, top:8, zIndex:20, display:"flex", gap:6, alignItems:"center" }}>
        ...
        <span className="sep" />
        <button title="Zoom Out" onClick={()=>zoomAt(1/1.1, centerPoint())}><i className="fa fa-magnifying-glass-minus"/></button>
        <span className="zoom"> {(view.k*100)|0}%</span>
        <button title="Zoom In" onClick={()=>zoomAt(1.1, centerPoint())}><i className="fa fa-magnifying-glass-plus"/></button>
        <button title="Show Entire Workspace" onClick={fitToContent}><i className="fa fa-rectangle-list"/></button>
      </div>
      ...
      <div
        ...
        style={{
          ...
          transformOrigin:"0 0", transform:`translate(${view.x}px, ${view.y}px) scale(${view.k})`
        }}
      >
          ...
          >
            ...
            <div className="yc-body">
              {Object.entries(n.fields).map(([key,val])=>{
                ...
                if(["bool","boolean","checkbox"].includes(t))
      ...
      {marq && (
        <div
          ...
          style={{
            position:"fixed", border:"1px dashed rgba(96,165,250,.9)", background:"rgba(96,165,250,.12)",
            pointerEvents:"none", zIndex:10,
            left:Math.min(marq.sxScr,marq.exScr), top:Math.min(marq.syScr,marq.eyScr),
            width:Math.abs(marq.exScr-marq.sxScr), height:Math.abs(marq.eyScr-marq.syScr)
          }}
        />
      )}
    </div>
